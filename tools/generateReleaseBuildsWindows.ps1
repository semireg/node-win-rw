...
foreach ($version in $node_versions) {
  ...
  node-pre-gyp configure --target=$version --module_name=win_rw --silent | Out-Null
...
foreach ($version in $electron_versions) {
  ...
  node-pre-gyp configure --target=$version --dist-url=https://electronjs.org/headers --module_name=win_rw --silent | Out-Null
