...
do
  ...
  node-pre-gyp configure --target=$version --module_name=win_rw --silent
...
do
  ...
  node-pre-gyp configure --target=$version --dist-url=https://electronjs.org/headers --module_name=win_rw --silent
